...
# python wer.py --char=1 --v=1 '/n/work1/juchen/whisper_decoding/text' 'Tokens' file_names... 
...
def wer(ref_file, hyp, hyp_fid, maxw=None, rt=None, cs=None, cluster=None, splitfile=None, ig=None, char=None, v=None, padding_symbol=None,):
    ...
    with codecs.open(ref_file, 'r', 'utf-8') as fh:
        for line in fh:
            # line = norm(line, line)
            if tochar:
    ...
    if hyp_fid not in rec_set:
        print("Skip")
        return None
    
    lab = rec_set[hyp_fid]
    hyp = norm(' '.join(lab), hyp)

    if tochar:
        array = characterize(hyp)
    else:
        array = hyp.rstrip('\n').split()
    ...
    if verbose:
        ...
        while lab1 < upper_lab or rec1 < upper_rec:
            if verbose > 1:
                print('lab(%s):' % hyp_fid.encode('utf-8'), end=' ')
            else:
                print('lab:', end=' ')
            ...
            print()
            if verbose > 1:
                print('rec(%s):' % hyp_fid.encode('utf-8'), end=' ')
            else:
                print('rec:', end=' ')
            ...
            lab1 = lab2
            rec1 = rec2
    return wer, result['ins'], result['sub'], result['del'], result['all']
    ...
    if verbose:
        ...
        if len(cluster_file) > 0:  # compute separated WERs for word clusters
            ...
            for line in open(cluster_file, 'r', encoding='utf-8'):
                for token in line.decode('utfß-8').rstrip('\n').split():
...
# print(wer(char=1, v=1,
#     ref_file="/n/work1/juchen/whisper_decoding/text",
#     hyp="Money managers such as Chase's Mr. Cantor also appeared willing to give the SEC the benefit of the doubt.",
#     hyp_fid='208c0101'))